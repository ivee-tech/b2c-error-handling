...
You can find the user journey and its orchestration steps in `TrustFrameworkBase.xml` with the Id `SignUpOrSignIn`. This repository’s active policy set also adds a pre‑AAD write REST validation (`REST-UserValidation`) inside the overridden `LocalAccountSignUpWithLogonEmail` technical profile (see `TrustFrameworkExtensions.xml`). That validation calls the backend `/users/validate` endpoint to block blocked or invalid accounts before creation.
...
<TechnicalProfile Id="LocalAccountSignUpWithLogonEmail">
  ...
  <ValidationTechnicalProfiles>
    <!-- Invoke REST user validation first -->
    <ValidationTechnicalProfile ReferenceId="REST-UserValidation" />
    <!-- Proceed with directory write only if validation succeeded -->
